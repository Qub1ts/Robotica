...
def predecir_marca(bgr, clf, rangos, area_min=300, umbral_conf=0.55,
    ...
    # Si la forma es alargada (circularidad < 0.45), es IMPOSIBLE que sea una marca.
    # Así forzamos al sistema a tratarlo como flecha y evitamos que se confundan.
    if feat[10] < 0.45:
        return None
...
class BrainFollowLine(Brain):
    ...
    FLECHA_ELONG_MIN          = 1.8
    FLECHA_ASIM_MIN           = 0.15
    ...
    def setup(self):
        ...
        # Decision de cruce bloqueada: una vez que elige salida,
        # no cambia hasta que aparezca una flecha nueva.
        self.locked_exit_side = None
        self.locked_exit_point = None
        self.arrow_generation = 0
    ...
    def elegir_salida(self, extremos, flecha):
        """
        Elige una salida y la bloquea.
        Si ya hay salida bloqueada, la mantiene aunque la deteccion de extremos varie.
        """
        ...
        # 1) Si ya habia una salida bloqueada, mantenerla.
        if self.locked_exit_side is not None:
            candidatas = [s for s in salidas if s['lado'] == self.locked_exit_side]

            if candidatas:
                # Si la misma salida sigue visible, usar su punto actualizado.
                self.locked_exit_point = candidatas[0]['punto']
                return candidatas[0]

            # Si por ruido no aparece ese lado este frame, inventamos una salida virtual
            # usando el ultimo punto conocido. Asi no cambia de decision.
            if self.locked_exit_point is not None:
                return {
                    'lado': self.locked_exit_side,
                    'punto': self.locked_exit_point,
                    'es_entrada': False,
                    'angulo': 0.0,
                }

        # 2) Si no hay flecha, no bloquear nada: seguir hacia arriba por defecto.
        if flecha is None:
            return min(salidas, key=lambda e: abs((e['angulo'] - 90 + 180) % 360 - 180))

        # 3) Elegir salida alineada con el vector centro->punta de la flecha.
        cx, cy = flecha['centro']
        px, py = flecha['punta']

        v_flecha = np.array([px - cx, py - cy], dtype=np.float32)
        nf = np.linalg.norm(v_flecha)

        if nf < 1e-6:
            return min(salidas, key=lambda e: abs((e['angulo'] - 90 + 180) % 360 - 180))

        v_flecha = v_flecha / nf
        ...
        for salida in salidas:
            ...
            v_salida = np.array([sx - cx, sy - cy], dtype=np.float32)
            ns = np.linalg.norm(v_salida)

            if ns < 1e-6:
                continue

            v_salida = v_salida / ns
            score = float(np.dot(v_flecha, v_salida))

            # No elegir volver por abajo.
            if salida['lado'] == 'abajo':
                score -= 1.0
        ...
        if mejor is not None:
            self.locked_exit_side = mejor['lado']
            self.locked_exit_point = mejor['punto']
            print(">>> SALIDA BLOQUEADA:", self.locked_exit_side,
                  "score=%.2f flecha=%.1f" % (mejor_score, flecha['angulo']))
    ...
    def actualizar_memoria_flecha(self, flecha_visual):
        """
        Guarda la ultima flecha valida.
        Si aparece una flecha nueva, libera la salida bloqueada para poder elegir otra.
        """
        if flecha_visual is None:
            return self.arrow_cache

        nueva = False

        if self.arrow_cache is None:
            nueva = True
        else:
            ang_old = self.arrow_cache.get('angulo', 0)
            ang_new = flecha_visual.get('angulo', 0)
            diff = abs((ang_new - ang_old + 180) % 360 - 180)

            # Si cambia bastante el angulo, asumimos flecha nueva.
            if diff > 25:
                nueva = True
        ...
        if nueva:
            self.locked_exit_side = None
            self.locked_exit_point = None
            self.arrow_generation += 1
            print(">>> NUEVA FLECHA: %.1f deg, libero salida bloqueada" %
                  flecha_visual['angulo'])
    ...
    def step(self):
        ...
        if self.avoiding:
            self.avoid_ticks += 1
            linea_centrada = (error is not None and abs(error) < self.AVOID_EXIT_ERR_MAX)
            timeout = self.avoid_ticks > self.AVOID_TICKS_MAX
            
            if (self.avoid_ticks > self.AVOID_TICKS_MIN and min_front > self.DIST_FRENTE_LIBRE and (linea_centrada or timeout)):
                self.avoiding   = False
        ...
        out = self.overlay(bgr, m_lin, m_rojo, extremos, salida_elegida, flecha_logica, marca_actual, error, v_cmd, w_cmd, estado)
        self._mostrar(out)
