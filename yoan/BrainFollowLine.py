...
def segmentar_escena(roi_imagen):
    ...
    rango_bajo = np.array([90, 80, 50])
    rango_alto = np.array([140, 255, 255])
...
def segmentar_marca(roi_imagen):
    ...
    # Definimos un rango de color azul con tolerancia para captar la línea incluso en condiciones difíciles
    rango_bajo = np.array([90, 80, 50])
    rango_alto = np.array([140, 255, 255])
    
    binaria = cv2.inRange(hsv, rango_bajo, rango_alto)
    
    # Tapar agujeros en la línea
    kernel_cierre = np.ones((7, 7), np.uint8)
    binaria = cv2.morphologyEx(binaria, cv2.MORPH_CLOSE, kernel_cierre)
    # Eliminar manchas que no fueron eliminadas por el filtro Gaussiano
    kernel_apertura = np.ones((5, 5), np.uint8)
    binaria = cv2.morphologyEx(binaria, cv2.MORPH_OPEN, kernel_apertura)
...
# --- NUEVAS FUNCIONES TOPOLÓGICAS ---
def categorizar_flecha(angulo):
    if angulo is None: 
        return None
    # 0 = Derecha, 90 = Abajo, 180 = Izquierda, 270 = Arriba (Delante)
    if 225 <= angulo <= 315:
        return "Delante"
    elif 135 < angulo < 225:
        return "Izquierda"
    elif 45 <= angulo <= 135:
        return "Atras" # Raro, pero por si acaso
    else: 
        return "Derecha"
...
def seleccionar_salida_logica(puntos_borde, direccion, roi_shape):
    ...
    # La entrada siempre es el punto más bajo en la pantalla (mayor Y)
    ...
    # Topología pura, evita errores matemáticos de distorsión
    if direccion == "Delante":
        return min(salidas, key=lambda p: abs(p[0] - ancho//2)) # El más centrado
    elif direccion == "Izquierda":
        return min(salidas, key=lambda p: p[0]) # El menor X (Más a la izquierda)
    elif direccion == "Derecha":
        return max(salidas, key=lambda p: p[0]) # El mayor X (Más a la derecha)
    else:
        return min(salidas, key=lambda p: p[1]) # Por defecto el más alto
# ------------------------------------
...
class BrainFollowLine(Brain):
    ...
    # Constantes PID y Variables de Estado
    Kp = -4.0
    Kd = -1.0
    Ki = 0.0   
    ...
    def setup(self):
        ...
        # Memoria de Control de Giro
        self.memoria_cruce_error = None
        self.frames_memoria = 0
        
        # Memoria de Visión (La Flecha)
        self.angulo_recordado = None
        self.frames_angulo = 0

        # Periodo refractario para evitar dobles detecciones al terminar un giro
    ...
    def step(self):
        ...
        if contornos_vision:
            ...
            # --- GESTIÓN DE CRUCES USANDO LA DIRECCIÓN LÓGICA ---
            # Solo procesamos el cruce si NO estamos en periodo de cooldown
            if "Cruce" in tipo_escena and self.angulo_recordado is not None and self.cooldown_cruce == 0:
                ...
                if salida_elegida:
                    ...
                    # --- NUEVA LÓGICA DE GATILLO ---
                    # En lugar de la media, buscamos la altura de la barra horizontal del cruce
                    salidas = [p for p in puntos_borde_vision if p != entrada_punto]
                    if salidas:
                        trigger_y = max(p[1] for p in salidas) # La Y más alta (más abajo en pantalla)
                    else:
                        trigger_y = entrada_punto[1]

                    umbral_y = int(alto * 0.60) 
                    cv2.line(cv_image, (0, umbral_y), (ancho, umbral_y), (0, 165, 255), 2)
                    # Dibujamos una línea azul fina para ver exactamente qué está usando como gatillo
                    cv2.line(cv_image, (0, trigger_y), (ancho, trigger_y), (255, 0, 0), 1)

                    if trigger_y > umbral_y:
                        # ESTAMOS EN EL CRUCE: GATILLO ACCIONADO
                        if direccion_logica == "Izquierda":
                            error_cruces = -0.6 # Volantazo seguro
                        ...
                        # Damos 25 frames de inercia ciega
                        self.frames_memoria = 30 
                        # Escudo visual de 50 frames desde el momento en que pisamos el cruce
                        self.cooldown_cruce = 25
                        
                        cv2.putText(cv_image, "EJECUTANDO SALIDA", (10, 125), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2)
                    else:
                        # VECTOR DE APROXIMACIÓN AL CRUCE
        ...
        # SEGUIMIENTO BASE
        # -------------------------------------------------------------
        foundLine, error_base = findLineDeviation(imageGray)

        # --- RECUPERACIÓN / CANCELACIÓN TEMPRANA DE MEMORIA ---
        # AHORA SÍ: Esto solo se ejecuta si estamos tirando de memoria
        if error_cruces is None and self.frames_memoria > 0:
            # Añadimos "self.frames_memoria < 20" para obligarle a girar a ciegas al menos 
            # 5 fotogramas antes de permitirle cazar la línea. Así evitamos que la cancele al instante.
        ...
        # Descuento del escudo de visión (se resta siempre que sea mayor que 0)
        ...
        # -------------------------------------------------------------
        # COMBINACIÓN DE SEÑALES Y SUAVIZADO 
        # -------------------------------------------------------------
